...
prediction accuracy but is not an accurate measure of the true performance. Typically
we want to evaluate on an independent test set and average over samples, not mini-batches.
The CIFAR-10 data set comes with a test set, which we have mis-used for validation
